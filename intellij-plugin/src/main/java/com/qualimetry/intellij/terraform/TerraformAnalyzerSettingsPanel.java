...
final class TerraformAnalyzerSettingsPanel {
    ...
    TerraformAnalyzerSettingsPanel() {
        ...
        importButton.addActionListener(e -> ImportFromSonarQubeAction.runImport(null, () ->
                loadFrom(TerraformAnalyzerSettings.getInstance())));
    ...
    void saveTo(@NotNull TerraformAnalyzerSettings settings) {
        ...
        if (settings.rulesReplaceDefaults) {
            for (RuleRow r : allRows) {
                ...
                rules.put(r.key, withExistingParams(new TerraformAnalyzerSettings.RuleOverride(enabled, severity), existing));
            }
        } else {
            for (RuleRow r : allRows) {
                RowState state = tableState.get(r.key);
    ...
    private static TerraformAnalyzerSettings.RuleOverride withExistingParams(
            TerraformAnalyzerSettings.RuleOverride next,
            TerraformAnalyzerSettings.RuleOverride existing) {
